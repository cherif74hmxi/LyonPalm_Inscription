...
@section('content')
    <section class="mb-6 card p-5">
        ...
        <p class="mt-1 text-sm text-slate-500">
            Retrouvez ici votre situation d inscription, certificat medical et paiements.
        </p>
    </section>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <article class="card p-5">
            ...
            <div class="mt-2 flex items-center gap-2">
                <span class="badge {{ $badgeCertificat }}">
                    {{ $statutCertificat === 'expire_bientot' ? 'Expire bientot' : ucfirst(str_replace('_', ' ', $statutCertificat)) }}
                </span>
            </div>
            @if ($certificat)
            ...
            @else
                <p class="mt-3 text-sm text-slate-600">Aucun certificat enregistre.</p>
            @endif
        </article>
        ...
        <article class="card p-5">
            ...
            @if ($adhesionActive)
                ...
                <p class="text-sm text-slate-600">Paye: {{ number_format((float) $adhesionActive->montant_paye, 2, ',', ' ') }} EUR</p>
                <p class="text-sm font-semibold text-slate-800">Solde: {{ number_format((float) $adhesionActive->solde, 2, ',', ' ') }} EUR</p>
            @else
                <p class="mt-2 text-sm text-slate-600">Aucune adhesion active pour le moment.</p>
            @endif
        </article>
        ...
        <article class="card p-5">
            ...
            <p class="text-sm text-slate-700">{{ $adherent->telephone ?: $adherent->mobile ?: 'Telephone non renseigne' }}</p>
    ...
    <section class="mt-6 grid gap-4 lg:grid-cols-2">
        <article class="card overflow-hidden">
            ...
            <div class="p-5">
                ...
                @else
                    <p class="text-sm text-slate-600">Aucun paiement enregistre sur la saison active.</p>
                @endif
            </div>
        </article>
        ...
        <article class="card overflow-hidden">
            <div class="border-b border-slate-100 px-5 py-4">
                <h2 class="text-lg font-bold text-slate-900">Mes adhesions recentes</h2>
            </div>
            <div class="p-5">
                ...
                @else
                    <p class="text-sm text-slate-600">Aucune adhesion historique.</p>
                @endif
            </div>
        </article>
    </section>
