...
@section('content')
    <div class="space-y-4">
        <section class="card p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Fiche adhérent et historiques.</p>
                </div>
                    ...
                    @if ($adherent->statut === 'actif')
                        <details class="relative">
                            ...
                            <div class="absolute right-0 z-20 mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-xl border border-slate-200 bg-white p-4 text-slate-900 shadow-2xl">
                                <h3 class="text-base font-bold">Confirmer l'archivage</h3>
                                <p class="mt-2 text-sm text-slate-600">Voulez-vous vraiment archiver cet adhérent ? Aucune suppression définitive ne sera faite.</p>
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Téléphone</p>
                    <p>{{ $adherent->mobile ?: '-' }}</p>
                </div>
                <div>
            ...
            @if ($adherent->representantLegal)
                <div class="mt-6 rounded-xl bg-slate-50 p-4">
                    <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-slate-500">Représentant légal</h2>
                    <p class="font-medium">{{ $adherent->representantLegal->prenom }} {{ $adherent->representantLegal->nom }}</p>
                    <p class="text-sm text-slate-600">{{ $adherent->representantLegal->lien_parental ?: 'Lien non précisé' }} - {{ $adherent->representantLegal->mobile ?: ($adherent->representantLegal->telephone ?: '-') }}</p>
                </div>
            @endif
        </section>

        <section class="card p-5">
            <h2 class="mb-3 text-lg font-semibold">Historique des certificats</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="table-head">
                        <tr>
                            <th class="px-3 py-2 text-left">Émission</th>
        ...
        <section class="card p-5">
            <h2 class="mb-3 text-lg font-semibold">Historique des adhésions et paiements</h2>
            ...
            <div class="space-y-4">
                @forelse ($adherent->adhesions as $adhesion)
                    ...
                    <article class="rounded-xl border border-slate-200 p-4">
                        ...
                        <p class="text-sm text-slate-600">Total : {{ number_format((float) $adhesion->montant_total, 2, ',', ' ') }} EUR - Payé : {{ number_format((float) $adhesion->montant_paye, 2, ',', ' ') }} EUR - Solde : {{ number_format((float) $adhesion->solde, 2, ',', ' ') }} EUR</p>
                ...
                @empty
                    <p class="text-sm text-slate-500">Aucune adhésion enregistrée.</p>
                @endforelse
            </div>
        </section>
    </div>
