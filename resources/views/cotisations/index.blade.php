...
@section('content')
    <section class="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-slate-500">Suivi des adhesions et paiements par saison.</p>
        </div>
    ...
    <section class="card mb-4 p-4">
        <form method="GET" class="grid gap-3 md:grid-cols-5">
            <select name="saison_id" class="input">
                <option value="">Saison active (defaut)</option>
            ...
            <select name="statut" class="input">
                ...
                <option value="a_jour" @selected(request('statut') === 'a_jour')>A jour</option>
                <option value="partiel" @selected(request('statut') === 'partiel')>Partiel</option>
                <option value="impaye" @selected(request('statut') === 'impaye')>Impaye</option>
            </select>
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                <a href="{{ route('adhesions.index') }}" class="btn-secondary">Reset</a>
            </div>
        </form>
    </section>
            ...
            <table class="min-w-full text-sm">
                <thead class="table-head">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left">Paye</th>
                ...
                <tbody>
                    @forelse ($adhesions as $adhesion)
                        @php $dernierPaiement = $adhesion->paiements->first(); @endphp
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-8 text-center text-slate-500">Aucune cotisation trouvee.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
